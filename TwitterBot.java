...
public class TwitterBot {
    ...
    public TwitterBot(BufferedReader br, int tweetColumn, NumberGenerator ng) {
        ...
        this.ng = ng;
        List<List<String>> tweetSent = TweetParser.csvDataToTrainingData(br, tweetColumn);
        for (List<String> sent: tweetSent) {
            mc.train(sent.iterator());
        }
    ...
    public String generateTweet(int numWords) {
        ...
        while (count < numWords) {
            while (mc.hasNext() && count < numWords) {
                String x = mc.next();
                if (!x.equals(MarkovChain.END_TOKEN)) {
                    tweet = tweet + " " + x;
                    count += 1;
                }
            }
