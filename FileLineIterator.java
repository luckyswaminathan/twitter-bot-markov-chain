...
public class FileLineIterator implements Iterator<String> {
    ...
    public static BufferedReader fileToReader(String filePath) {
        ...
        try {
            Reader red = new FileReader(filePath);
            b = new BufferedReader(red);
        } catch (Exception x) {
            throw new IllegalArgumentException("filepath doesn't work");
        }
        return b;
    }
    ...
    public String next() {
        if (!iter.hasNext()) {
            throw new java.util.NoSuchElementException("There is no next line");
        }
        return iter.next();
