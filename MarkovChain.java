...
public class MarkovChain implements Iterator<String> {
    ...
    void addBigram(String first, String second) {
        ...
        if (chain.get(first) != null) {
            ProbabilityDistribution<String> x = chain.get(first);
            x.record(second);
            chain.replace(first, x);
        } else {
            ProbabilityDistribution<String> newP = new ProbabilityDistribution<String>();
            newP.record(second);
            chain.put(first, newP);
        }
    ...
    public void train(Iterator<String> sentence) {
        ...
        while (sentence.hasNext()) {
            String x = sentence.next();
            addBigram(s,x);
            s = x;
        }
        addBigram(s, END_TOKEN);
    }
    ...
    public boolean hasNext() {
        if (currentWord.equals(END_TOKEN)) {
            return false;
        } else if (chain.containsKey(currentWord)) {
            return true;
        } else {
            return justReset;
        }
    ...
    public String next() {
        ...
        if (hasNext()) {
            ProbabilityDistribution<String> prob = chain.get(currentWord);
            String picked = prob.pick(ng);
            currentWord = picked;
            return picked;
        } else {
