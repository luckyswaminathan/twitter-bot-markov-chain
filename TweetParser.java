...
public class TweetParser {
    ...
    static String extractColumn(String csvLine, int csvColumn) {
        ...
        if (csvColumn > -1 && csvColumn < s.length) {
            return s[csvColumn];
        }
        return null;
    }
    ...
    static List<String> csvDataToTweets(BufferedReader br, int tweetColumn) {
        ...
        while (f.hasNext()) {
            String next = f.next();
            String neC = extractColumn(next, tweetColumn);
            if (neC != null && next.length() > 0) {
                tweets.add(neC);
            }
        }
        return tweets;
    ...
    static List<String> parseAndCleanSentence(String sentence) {
        ...
        List<String> splitted2 = new LinkedList<String>();
        for (int i = 0; i < splitted.length; i++) {
            String s = cleanWord(splitted[i]);
            if (s != null && !(s.isEmpty())) {
                splitted2.add(s);
            }
        }
    ...
    static List<List<String>> parseAndCleanTweet(String tweet) {
        ...
        List<List<String>> tweetP = new LinkedList<>();
        if (s.equals("")) {
            return tweetP;
        }
        List<String> tweets = tweetSplit(s);
        for (String m : tweets) {
            tweetP.add(parseAndCleanSentence(m));
        }
    ...
    public static List<List<String>> csvDataToTrainingData(
        ...
        List<String> allTweets = csvDataToTweets(br, tweetColumn);
        for (int i = 0; i < allTweets.size(); i++) {
            if (allTweets.get(i) != null) {
                List<List<String>> cleaned = parseAndCleanTweet(allTweets.get(i));
                if (cleaned.size() != 0) {
                    dataSet.addAll(cleaned);
                }
            }
        }
